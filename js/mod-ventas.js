...
async function renderVentasGanancias() {
  // Actualizar banner de ganancias totales
  const ventas   = await DB.ventas();
  const tiendas  = await DB.tiendas();
  const m        = mes();
  const ventasMes = ventas.filter(v=>v.fecha_venta?.startsWith(m));

  const totalGan    = ventasMes.reduce((s,v)=>s+calcVenta(v).ganancia,0);
  const totalVenta  = ventasMes.reduce((s,v)=>s+calcVenta(v).totalVenta,0);
  const totalCostos = ventasMes.reduce((s,v)=>s+calcVenta(v).totalCostos,0);
  ...
  if(elCnt) elCnt.textContent = ventasMes.length;

  // Chips por tienda — ganancia del mes + total de ventas acumuladas
  const chipEl = document.getElementById('vg-por-tienda');
  if(chipEl) chipEl.innerHTML = tiendas.map(t=>{
    const tvMes  = ventasMes.filter(v=>v.tienda_id===t.id);          // solo mes actual
    const tvTodo = ventas.filter(v=>v.tienda_id===t.id);             // todas las ventas
    const g   = tvMes.reduce((s,v)=>s+calcVenta(v).ganancia,0);
    const ing = tvMes.reduce((s,v)=>s+calcVenta(v).totalVenta,0);
    const mar = ing>0?(g/ing)*100:0;
    ...
    ">
      ...
      <div style="font-size:12px;color:var(--text3);margin-top:2px;">
        <b>${tvTodo.length}</b> ventas total
      </div>
    </div>`;
  }).join('');
